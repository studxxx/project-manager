...
class ActionFetcher
{
    ...
    public function all(Filter $filter, int $page, int $size): PaginationInterface
    {
        ...
        if ($filter->member) {
            $qb->innerJoin(
                'project',
                'work_projects_project_memberships',
                'membership',
    ...
    private function createQb(): QueryBuilder
    {
        return $this->connection->createQueryBuilder()
            ->select(
                ...
                'TRIM(CONCAT(set_revoked_executor.name_first, \' \', set_revoked_executor.name_last)) AS
                 set_revoked_executor_name',
            ...
            ->leftJoin(
                'c',
                'work_members_members',
                'set_revoked_executor',
                'c.set_revoked_executor_id = set_executor.id'
            )
            ->leftJoin('c', 'work_projects_tasks', 'task', 'c.task_id = task.id')
            ->leftJoin('c', 'work_projects_projects', 'project', 'task.project_id = project.id')
